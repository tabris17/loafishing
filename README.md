...
网站列表中，每条匹配记录对应一行。匹配记录可以是完整的 URL ，也可以省略协议和路径，支持 * 号通配符。匹配记录也可以是用 / 斜杠包裹的正则表达式。
...
有时候图片或视频会被其他透明的 HTML 元素遮盖，虽然图片或视频在视口中可见，但是无法响应鼠标的 Hover 事件。可以在移动鼠标时按下 Ctrl 键，扩展会尝试找到被遮挡的图片或视频元素。
